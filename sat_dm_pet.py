...
class ModelBuilder:
    ...
    def convolution_block(self, x, filters, kernel_size=(3, 3, 3), strides=(1, 1, 1)):
        x = Conv3D(filters, kernel_size, strides=strides, padding='same',
                   kernel_regularizer=l2(6.7528639273478225e-06))(x)
    ...
    def create_model(self):
        ...
        x = self.convolution_block(input_img, 16)
        conv1_out = x
        x = self.context_module(x, 16)
        x = Add()([x, conv1_out])

        # Conv2 block (32 filters, stride 2)
        x = self.convolution_block(x, 32, strides=(2, 2, 2))
        conv2_out = x
        x = self.context_module(x, 32)
        x = Add()([x, conv2_out])

        # Conv3 block (64 filters, stride 2)
        x = self.convolution_block(x, 64, strides=(2, 2, 2))
        conv3_out = x
        x = self.context_module(x, 64)
        x = Add()([x, conv3_out])

        # Conv4 block (128 filters, stride 2)
        x = self.convolution_block(x, 128, strides=(2, 2, 2))
        conv4_out = x
        x = self.context_module(x, 128)
        x = Add()([x, conv4_out])

        # Conv5 block (256 filters, stride 2)
        x = self.convolution_block(x, 256, strides=(2, 2, 2))
        conv5_out = x
        x = self.context_module(x, 256)
        x = Add()([x, conv5_out])
...
class Trainer:
    ...
    def augment_data(self, X, flip_prob=0.1, rotate_prob=0.1):
    ...
    def train_model(self, X, Y):
        ...
        for fold_num, (train_idx, val_idx) in enumerate(stratified_kfold.split(X, Y.argmax(axis=1))):
            ...
            model.compile(optimizer=Adam(learning_rate=7.767449713530702e-05),
