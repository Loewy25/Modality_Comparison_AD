...
# Function to overlay Grad-CAM heatmap onto original image slices and save the images
def save_gradcam(heatmap, original_img,
    ...
    heatmap_max = np.max(upsampled_heatmap)
    if heatmap_max != 0:
        upsampled_heatmap /= heatmap_max
    ...
    # For each axis, find the slice index with maximum activation
    for axis in axes:
        axis_idx = axis_indices[axis]
        # Sum the heatmap over the other two axes to get activation along this axis
        summed_heatmap = np.sum(upsampled_heatmap, axis=tuple(i for i in range(3) if i != axis_idx))
        # Find the index with maximum activation
        max_index = np.argmax(summed_heatmap)
        slice_indices[axis] = max_index

    # Now, generate and save overlay images for each axis
    for axis in axes:
        axis_idx = axis_indices[axis]
        slice_index = slice_indices[axis]
        
        if axis == 'x':
            img_slice = original_data[slice_index, :, :]
            heatmap_slice = upsampled_heatmap[slice_index, :, :]
        ...
        # Normalize the image slice for display
        img_slice_normalized = (img_slice - np.min(img_slice)) / (np.max(img_slice) - np.min(img_slice) + 1e-8)

        plt.figure(figsize=(8, 8))
        plt.imshow(img_slice_normalized, cmap='gray', origin='lower')
        plt.imshow(heatmap_slice, cmap='jet', alpha=0.5, origin='lower')
        plt.axis('off')

        # Save the overlay image
        overlay_file_name = f"overlay_{task}_{modality}_class{class_idx}_{layer_name}_{axis}_slice{slice_index}.png"
        overlay_save_path = os.path.join(save_dir, overlay_file_name)
        plt.savefig(overlay_save_path, bbox_inches='tight', pad_inches=0)
        plt.close()
        print(f'Overlay image saved at {overlay_save_path}')

# Function to apply Grad-CAM for the last convolutional layer for all dataset images
def apply_gradcam_single_layer(model, imgs, original_imgs,
                               task, modality, info):
    # Get the name of the last Conv3D layer
    last_conv_layer_name = [layer.name for layer in model.layers if isinstance(layer, Conv3D)][-1]
    ...
    for class_idx in range(2):  # Loop through both class indices (class 0 and class 1)
        for i, img in enumerate(imgs):
            ...
            print(f"Heatmap shape for layer {last_conv_layer_name} and class {class_idx}: {heatmap.shape}")
            # Use the corresponding original image
            original_img = original_imgs[i]
            # Save and visualize the heatmap
            save_gradcam(heatmap, original_img,
                         task, modality, last_conv_layer_name, class_idx, info)
...
def augment_data(X):
    ...
    for img in X:
        img_aug = img.copy()
        # Randomly flip along each axis with 5% probability
...
if __name__ == '__main__':
    ...
    info = 'totally_train'  # Additional info for saving results
    ...
    # Apply Grad-CAM to all images using the last convolutional layer and overlay onto original images
    apply_gradcam_single_layer(best_model, imgs, original_imgs, task, modality, info)
