...
image_mri,label,masker=loading_mask('pc','PET')
#image_pet,label,masker=loading_mask('cd','PET')

start_time = time.time()  # Capture start time

performance_dict_mri,all_y_test_mri, all_y_prob_mri, all_predictions_mri=nested_crossvalidation(image_mri, label, 'PET', 'pc')
end_time = time.time()  # Capture end time
